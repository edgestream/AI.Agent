...
- `agents.json` is the frontend server configuration for available AG-UI agents.
- `AI.Agent.slnx` is the root .NET solution. Plain `dotnet build` should build the .NET library and samples.
...
- `samples/dotnet/NewsAgent` is the current .NET AG-UI sample.
...
7. Run `dotnet build` when touching `AI.Agent.slnx`, `dotnet/**`, `samples/dotnet/**`, .NET launch profiles, or shared .NET configuration.
