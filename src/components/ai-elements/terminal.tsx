...
export const TerminalContent = ({
  ...
  const { output, chunks, isStreaming, autoScroll } =
        ...
        <pre className="whitespace-pre-wrap break-words">
          {chunks ? (
            chunks.map((chunk, index) => (
              <span
                className={chunk.isError ? "text-red-400" : undefined}
                key={index}
              >
                <Ansi>{chunk.output}</Ansi>
              </span>
            ))
          ) : (
            <Ansi>{output}</Ansi>
...
export const Terminal = ({
  ...
  const contextValue = useMemo(
    () => ({ autoScroll, chunks, isStreaming, onClear, output }),
    [autoScroll, chunks, isStreaming, onClear, output]
  );
